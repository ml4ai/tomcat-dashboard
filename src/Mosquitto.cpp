...
void Mosquitto::mosquitto_callback_on_connect(struct mosquitto *mqtt_client,
  ...
  case 0:
    ...
    break;
  case 1:
    mosquitto->on_error("Connection refused (unacceptable protocol version)");
    break;
  case 2:
...
void Mosquitto::connect(const string &address, int port, int alive_delay,
  ...
  while (!this->running && trials > 0) {
    ...
    int error_code = mosquitto_connect(this->mqtt_client, address.c_str(), port,
                                       alive_delay);
...
void Mosquitto::publish(const string &topic, const string &message) {
  ...
  const int qos = 0;
  int error_code = mosquitto_publish(this->mqtt_client, NULL, topic.c_str(),
                                     len, (const void *)payload, qos, false);
  if (error_code != MOSQ_ERR_SUCCESS) {
    stringstream ss;
